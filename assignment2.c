...
//Communication
      ...
      sbuffer_top[0] = mpi_myrank;
      sbuffer_bottom[0] = mpi_myrank;
      ...
      {
        g_GOL_CELL[0][i] = rbuffer_top[i];
        g_GOL_CELL[g_y_cell_size - 1][i] = rbuffer_bottom[i];
      }
...
{
  char filename[256];
  sprintf(filename, "cellgraph_rank%d_%d.txt", 
  ...
  for (i = 0;i < g_y_cell_size;++i)
  {
    ...
    {
      fprintf(fp, "%d ", g_GOL_CELL[i][j]);
    }
    fprintf(fp, "%d;\n ", g_GOL_CELL[i][g_x_cell_size - 1]);
  }
